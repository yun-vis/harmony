...
void Pathway::initLayout( map< unsigned int, Polygon2 > &polygonComplex ) {
	...
	for( unsigned int i = 0; i < _layoutSubGraph.size(); i++ ) {
		...
		BGL_FORALL_VERTICES( vd, _layoutSubGraph[ i ], ForceGraph ) {
				...
				_layoutSubGraph[ i ][ vd ].coordPtr->x() =
						half_w * ( _layoutSubGraph[ i ][ vd ].coordPtr->x() - minX ) / ( maxX - minX ) + centroid.x() -
						scale * half_w;
				_layoutSubGraph[ i ][ vd ].coordPtr->y() =
						half_h * ( _layoutSubGraph[ i ][ vd ].coordPtr->y() - minY ) / ( maxY - minY ) + centroid.y() -
						scale * half_h;
			}
	}
}
...
void Pathway::genGraph( void ) {
	...
	for( unsigned int i = 0; i < _react.size(); i++ ) {
		...
		for( unsigned int j = 0; j < _react[ i ].reactantVec.size(); j++ ) {
			...
			if( flag ) {
				...
				_graph[ metaVD ].subsystems.insert(
						pair< string, Subdomain * >( _react[ i ].subsystem->name, _react[ i ].subsystem ) );
				_graph[ metaVD ].metaPtr->subsystems.insert(
						pair< string, Subdomain * >( _react[ i ].subsystem->name, _react[ i ].subsystem ) );
	...
	cerr << "density_before = " << 100.0 * ( double ) _nHyperEtoE /
	                               ( double ) ( ( _nV.size() + _react.size() ) * ( _nV.size() + _react.size() - 1 ) )
	     << endl;
	cerr << "density_after = " << 100.0 * ( double ) num_edges( _graph ) /
	                              ( double ) ( num_vertices( _graph ) * ( num_vertices( _graph ) - 1 ) ) << endl;
						...
						for( itS = _graph[ vdS ].subsystems.begin(); itS != _graph[ vdS ].subsystems.end(); itS++ ) {
							if( ( itS->first == iter->first ) ) {
								...
								if( iter != idMap.end() ) {
									...
									g[ newSVD ].type = _graph[ vdS ].type;
									g[ newSVD ].coordPtr = new Coord2( _graph[ vdS ].coordPtr->x(),
									                                   _graph[ vdS ].coordPtr->x() );
									g[ newSVD ].widthPtr = _graph[ vdS ].widthPtr;
									g[ newSVD ].heightPtr = _graph[ vdS ].heightPtr;
									idMap.insert(
											pair< unsigned int, unsigned int >( _graph[ vdS ].id, g[ newSVD ].id ) );
									//nVertices++;
									nDummies++;
									//cerr << *g[ newSVD ].namePtr << ", ";
								}
						...
						for( itT = _graph[ vdT ].subsystems.begin(); itT != _graph[ vdT ].subsystems.end(); itT++ ) {
							if( ( itT->first == iter->first ) ) {
								...
								if( iter != idMap.end() ) {
									...
									g[ newTVD ].type = _graph[ vdT ].type;
									g[ newTVD ].coordPtr = new Coord2( _graph[ vdT ].coordPtr->x(),
									                                   _graph[ vdT ].coordPtr->y() );
									g[ newTVD ].widthPtr = _graph[ vdT ].widthPtr;
									g[ newTVD ].heightPtr = _graph[ vdT ].heightPtr;
									idMap.insert(
											pair< unsigned int, unsigned int >( _graph[ vdT ].id, g[ newTVD ].id ) );
									//nVertices++;
									nDummies++;
...
void Pathway::genLayoutSubGraphs( void ) {
	...
	for( unsigned int i = 0; i < _subGraph.size(); i++ ) {
		...
		BGL_FORALL_VERTICES( vd, _subGraph[ i ], MetaboliteGraph ) {
				...
				_layoutSubGraph[ i ][ vdL ].prevCoordPtr = new Coord2( _subGraph[ i ][ vd ].coordPtr->x(),
				                                                       _subGraph[ i ][ vd ].coordPtr->y() );
		...
		BGL_FORALL_EDGES( ed, _subGraph[ i ], MetaboliteGraph ) {
				...
				if( found == false ) {
					...
					pair< ForceGraph::edge_descriptor, unsigned int > foreE = add_edge( vdSL, vdTL,
					                                                                    _layoutSubGraph[ i ] );
					ForceGraph::edge_descriptor edL = foreE.first;
...
void Pathway::planar_graph_embedding( vector< SkeletonGraphVVPair > &addedED ) {
	...
	for( unsigned int i = 0; i < addedED.size(); i++ ) {
		pair< UndirectedPropertyGraph::edge_descriptor, int > foreE = add_edge( _skeletonGraph[ addedED[ i ].first ].id,
		                                                                        _skeletonGraph[ addedED[ i ].second ].id,
		                                                                        propG );
	...
	BGL_FORALL_VERTICES( vd, propG, UndirectedPropertyGraph ) {
			...
			for( ; itE != embedding[ vd ].end(); itE++ ) {
				...
				if( vertexIndex[ vd ] == vertexIndex[ vdS ] )
					vdVec.push_back( vertex( vertexIndex[ vdT ], _skeletonGraph ) );
...
bool Pathway::loop_in_dual_graph_test( UndirectedPropertyGraph &propG ) {
	...
	BGL_FORALL_EDGES( ed, dualG, UndirectedPropertyGraph ) {
			...
			if( dualVertexIndex[ vdS ] == 0 || dualVertexIndex[ vdT ] == 0 ) {
				delEDVec.insert(
						pair< UndirectedPropertyGraph::edge_descriptor, UndirectedPropertyGraph::edge_descriptor >( ed,
						                                                                                            ed ) );
			}
			if( vdS == vdT )
				delEDVec.insert(
...
bool Pathway::constrained_planarity_test( vector< SkeletonGraphVVPair > &addedED,
	...
	pair< UndirectedPropertyGraph::edge_descriptor, unsigned int > foreE = add_edge( _skeletonGraph[ newE.first ].id,
	                                                                                 _skeletonGraph[ newE.second ].id,
	                                                                                 propG );
				...
				for( tie( eo, eo_end ) = out_edges( vd, _graph ); eo != eo_end; ++eo ) {
					...
					if( _graph[ vdT ].type == "reaction" ) {
						...
						if( dIter != domainFreq.end() ) {
							...
							domainFreq.insert(
									pair< unsigned int, unsigned int >( _graph[ vdT ].reactPtr->subsystem->id, 1 ) );
						}
					}
				}
				...
				for( tie( ei, ei_end ) = in_edges( vd, _graph ); ei != ei_end; ++ei ) {
					...
					if( _graph[ vdS ].type == "reaction" ) {
						...
						if( dIter != domainFreq.end() ) {
							...
							domainFreq.insert(
									pair< unsigned int, unsigned int >( _graph[ vdS ].reactPtr->subsystem->id, 1 ) );
						}
					}
				}
				...
				for( map< unsigned int, unsigned int >::iterator oIter = domainFreq.begin();
					...
					for( ; iIter != domainFreq.end(); iIter++ ) {
						...
						if( found == true ) {
							...
							pair< SkeletonGraph::edge_descriptor, unsigned int > foreE = add_edge( vdS, vdT,
							                                                                       _skeletonGraph );
