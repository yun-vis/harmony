...
void Cell::_buildConnectedComponent( void ) {
	...
	for( unsigned int i = 0; i < lsubg.size(); i++ ) {
		...
		for( unsigned int j = 0; j < num; j++ ) {
			...
			double multiple = ceil(
					pow( ceil( totalArea / veCoverage / ( double ) _paramUnit ), powUnit ) );    // text area size
			if( unit > multiple ) {
				unit = multiple;
			}
		}
		...
		for( unsigned int j = 0; j < num; j++ ) {
			...
			double veCoverage = num_vertices( lsubg[ i ] ) + ( *_veRatioPtr ) * num_edges( lsubg[ i ] );
			cc[ j ].multiple = ceil( pow( ceil( totalArea / veCoverage / ( double ) _paramUnit ), powUnit ) /
			                         unit );    // text area size
			...
			for( unsigned int k = 0; k < cc[ j ].lsubgVec.size(); k++ ) {
				...
				itC->second.detail.forceGraph()[ vdNew ].prevCoordPtr = new Coord2(
