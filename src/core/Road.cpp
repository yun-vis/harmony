...
#include "core/Road.h"
#include "../../third_party/steinertree/include.h"
                      ...
                      vector < Highway > *highwayRoadPtr )
{
    ...
    for( unsigned int i = 0; i < _terminalVec.size(); i++ ){
        ...
        for( it = _terminalVec[i].common.begin();
             it != _terminalVec[i].common.end(); it++ ){
            cerr << *_pathwayPtr->g()[ it->first ].namePtr << ", ";
        }
        cerr << endl;
#endif // DEBUG
    }
...
#endif // DEBUG
				polygonVD.insert(
						pair< unsigned int, vector< UndirectedBaseGraph::vertex_descriptor > >( fg[ vd ].initID,
						                                                                        polygons ) );
			}
		}
		...
		for( itC = cellComponent.begin(); itC != cellComponent.end(); itC++ ) {
			...
			for( unsigned int i = 0; i < seedVec.size(); i++ ) {
				...
				for( unsigned int j = 0; j < size; j++ ) {
					...
					Coord2 coordS( p.elements()[ j ].x(), p.elements()[ j ].y() );
					Coord2 coordT( p.elements()[ ( j + 1 ) % ( int ) size ].x(),
					               p.elements()[ ( j + 1 ) % ( int ) size ].y() );
					UndirectedBaseGraph::vertex_descriptor vdS = NULL, vdT = NULL;
					_findVertexInRoad( coordS, vdS );
					_findVertexInRoad( coordT, vdT );
					...
					if( found ) {
						...
						if( vdVec.size() == 0 ) {
							...
							if( found ) {
								...
								pair< UndirectedBaseGraph::edge_descriptor, unsigned int > foreE = add_edge( vdS, vdT,
								                                                                             _road );
								UndirectedBaseGraph::edge_descriptor foreED = foreE.first;
								_road[ foreED ].id = nEdges;
								_road[ foreED ].visitedTimes = 0;
							...
							for( unsigned int k = 0; k < vdVec.size(); k++ ) {
								double dist = ( *_road[ vdVec[ k ] ].coordPtr - coordS ).norm();
								vdMap.insert(
										pair< double, UndirectedBaseGraph::vertex_descriptor >( dist, vdVec[ k ] ) );
							}
							...
							for( ; itN != vdMap.end(); itN++ ) {
								...
								if( found ) {
									...
									pair< UndirectedBaseGraph::edge_descriptor, unsigned int > foreE = add_edge(
											itM->second, itN->second, _road );
									UndirectedBaseGraph::edge_descriptor foreED = foreE.first;
									_road[ foreED ].id = nEdges;
									_road[ foreED ].visitedTimes = 0;
									_road[ foreED ].weight = ( *_road[ itM->second ].coordPtr -
									                           *_road[ itN->second ].coordPtr ).norm();
	...
	for( unsigned int i = 0; i < nSystems; i++ ) {
		...
		BGL_FORALL_VERTICES( vd, subg[ i ], MetaboliteGraph ) {
				if( ( *subg[ i ][ vd ].isSelectedPtr == true ) && ( *subg[ i ][ vd ].selectedIDPtr == _selectedID ) ) {
					aliasVDMapO.insert(
							pair< MetaboliteGraph::vertex_descriptor, MetaboliteGraph::vertex_descriptor >( vd, vd ) );
				}
			}
		...
		for( unsigned int j = i + 1; j < nSystems; j++ ) {
			...
			BGL_FORALL_VERTICES( vd, subg[ j ], MetaboliteGraph ) {
					...
					if( ( *subg[ j ][ vd ].isSelectedPtr == true ) &&
					    ( *subg[ j ][ vd ].selectedIDPtr == _selectedID ) ) {
						aliasVDMapI.insert(
								pair< MetaboliteGraph::vertex_descriptor, MetaboliteGraph::vertex_descriptor >( vd,
								                                                                                vd ) );
					}
				}
