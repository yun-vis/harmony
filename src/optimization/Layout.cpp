...
//      none
//
void Layout::_init( SkeletonGraph *__skeletonGraph ) {
	_skeletonGraph = __skeletonGraph;
...
//  	Specific functions
//------------------------------------------------------------------------------
void Layout::up_degrees( SkeletonGraph::vertex_descriptor &vdC, int &degrees, bool &isAllVisited ) {
	degrees = 0;
	isAllVisited = true;
	SkeletonGraph &dependG = *_skeletonGraph;
...
bool
Layout::subGraphConnected( SkeletonGraph::vertex_descriptor &vdA, SkeletonGraph::vertex_descriptor &vdB, bool &visit ) {
	SkeletonGraph &dependG = *_skeletonGraph;
	bool isConnected = false;
	
	for( unsigned int i = 0; i < _childSubGraph[ dependG[ vdA ].id ].size(); i++ ) {
		
		SkeletonGraph::vertex_descriptor vdC = _childSubGraph[ dependG[ vdA ].id ][ i ];
...
void Layout::findChildSubGraph( SkeletonGraph::vertex_descriptor &vdTarget ) {
	SkeletonGraph &dependG = *_skeletonGraph;
	...
	stack< SkeletonGraph::vertex_descriptor > vdStack;
	vdStack.push( vdTarget );
	SkeletonGraph::vertex_descriptor currVD = vdTarget;
	...
	while( vdStack.size() != 0 ) {
		...
		vector< SkeletonGraph::vertex_descriptor > neighborVD;
		SkeletonGraph::out_edge_iterator eo, eo_end;
...
void Layout::radialPlacement( void ) {
	SkeletonGraph &dependG = *_skeletonGraph;
	SkeletonGraph::vertex_descriptor refVD;
	double maxZone = 0.0;
	...
	map< unsigned int, SkeletonGraph::vertex_descriptor > sortedLeaves;
	unsigned int leafNum = 0;
	stack< SkeletonGraph::vertex_descriptor > vdStack;
	vdStack.push( refVD );
	// cerr << " push( " << dependG[ refVD ].id << " ) ";
	SkeletonGraph::vertex_descriptor currVD = refVD;
	...
	while( vdStack.size() != 0 ) {
		...
		if( neighborVD.size() == 0 ) {
			...
			if( isOuter == true ) {
				sortedLeaves.insert( pair< unsigned int, SkeletonGraph::vertex_descriptor >( leafNum, currVD ) );
				leafNum++;
			}
			vdStack.pop();
	...
	BGL_FORALL_VERTICES( vd, dependG, SkeletonGraph ) {
			if( out_degree( vd, dependG ) == 0 ) {
				...
				sortedLeaves.insert( pair< unsigned int, SkeletonGraph::vertex_descriptor >( leafNum, vd ) );
				leafNum++;
			}
		}
	
	// assign the angles to leaves
	map< unsigned int, SkeletonGraph::vertex_descriptor >::iterator itL = sortedLeaves.begin();
	int index = 0;
		...
		BGL_FORALL_VERTICES( vd, dependG, SkeletonGraph ) {
				...
				if( dependG[ vd ].angle == 0 && dependG[ vd ].zone == ( int ) i ) {
					...
					if( count == 0 )
						...
						dependG[ vd ].angle = sum / ( double ) count +
						                      2.0 * M_PI / 180.0 * ( ( rand() % ( int ) 180 ) / 180.0 - 0.5 );
				}
			}
	}
