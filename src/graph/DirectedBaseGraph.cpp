...
//------------------------------------------------------------------------------
//	Customized DirectedBaseGraph Functions
//------------------------------------------------------------------------------
//
//  DirectedBaseGraph::printGraph -- print the graph.
//
//  Inputs
//  g   : object of Grpah
//
...
//  none
//
void printGraph( DirectedBaseGraph &graph ) {
	...
	BGL_FORALL_VERTICES( vd, graph, DirectedBaseGraph ) {
		...
		DirectedBaseGraph::degree_size_type      degrees         = out_degree( vd, graph );
	}
#endif  // DEBUG

#ifdef  DEBUG
	// print edge information
	BGL_FORALL_EDGES( ed, graph, DirectedBaseGraph ) {

		DirectedBaseGraph::vertex_descriptor vdS = source( ed, graph );
		DirectedBaseGraph::vertex_descriptor vdT = target( ed, graph );
...
//
//  DirectedBaseGraph::clearGraph -- clear the graph.
//
//  Inputs
//  g   : object of Grpah
//
...
//  none
//
void clearGraph( DirectedBaseGraph &graph ) {
	// clear edges
	DirectedBaseGraph::edge_iterator ei, ei_end, e_next;
	...
	// clear vertices
	pair< DirectedBaseGraph::vertex_iterator, DirectedBaseGraph::vertex_iterator > vp;
	for( vp = vertices( graph ); vp.first != vp.second; ) {
		DirectedBaseGraph::vertex_descriptor vd = ( *vp.first );
		++vp.first;
